...
module Data.Environment(Env,empty,lookup,insert,insertWith,fromList,toList) where
...
instance (Eq a, Hashable a, PreOrd b) => PreOrd (Env a b) where
...
lookup :: (Eq a, Hashable a) => a -> Env a b -> Maybe b
lookup a (Env m) = H.lookup a m

insert :: (Eq a, Hashable a) => a -> b -> Env a b -> Env a b
insert a b (Env m) = Env (H.insert a b m)

insertWith :: (Eq a, Hashable a) => (b -> b -> b) -> a -> b -> Env a b -> Env a b
insertWith f a b (Env m) = Env (H.insertWith f a b m)

fromList :: (Eq a, Hashable a) => [(a,b)] -> Env a b
