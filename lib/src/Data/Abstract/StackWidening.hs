...
type IterationStrategy stack fun a b = (stack a,fun a b) -> a -> ((stack a,fun a b),Compute a b)

-- | Datatype that signals that we are in a loop.
data Compute a b = Compute a | ComputeAndStabilize a | Cached b deriving (Show,Eq)
