...
spec = do
  ...
  describe "Chaotic"  (sharedSpec (\f -> snd . Arrow.run (toChaotic f) id ?stackWiden (T.widening ?widen)))
...
toChaotic :: (Show a, Show b, Identifiable a, Complete b, ?stackWiden :: StackWidening stack a)
          => Arr a b -> TerminatingT (ChaoticT a (Terminating b) (StackT stack a (Terminating b) (->))) a b
toChaotic x = x
{-# INLINE toChaotic #-}
