...
-- type MethodReader = (Maybe MethodSignature, Int, [CatchClause])
--
...
--
-- instance ArrowTryCatch Failure x y z Interp where
...
-- deriving instance ArrowFail Failure Interp
...
--
-- type CanFail c = (ArrowChoice c, ArrowFail Failure c)
...
-- type CanCatch x c = ArrowTryCatch Failure x (Maybe Val) (Maybe Val) c
--
...
--   case v of
--     Just y -> returnA -< y
--     Nothing -> failA -< printf "Variable %s not bounded" (show x)
--
...
--   case v of
--     Just y -> returnA -< y
--     Nothing -> failA -< printf "Address %s not bounded" (show x)
--
...
--     Nothing -> failA -< printf "Field %s not bound" (show x)
--
...
--     Just x -> returnA -< x
--     Nothing -> failA -< printf "CompilationUnit %s not loaded" (show n)
--
...
--       _ -> failA -< printf "Method %s not defined for class %s" (show n) (show c)
--     Nothing -> failA -< printf "Undefined class %s" (show c)
--
...
--       v' <- read' -< addr
--       returnA -< (addr, v')
--     _ -> failA -< printf "Variable %s is not a reference" (show localName)
--
...
--     _ -> failA -< printf "Variable %s not bound to an array" (show localName)
--
...
--     _ -> failA -< printf "Variable %s not bound to an object" (show localName)
--
-- getCurrentMethod :: (CanFail c, CanUseReader c) => c () MethodSignature
-- getCurrentMethod = proc () -> do
--   (m, _, _) <- askA -< ()
--   case m of
--     Just m' -> returnA -< m'
--     Nothing -> failA -< "No method running"
--
-- getCurrentMethodBody :: (CanFail c, CanUseReader c, CanUseConst) => c () MethodBody
-- getCurrentMethodBody = proc () -> do
--   s <- getCurrentMethod -< ()
--   m <- fetchMethod -< s
--   case methodBody m of
--     EmptyBody -> failA -< printf "Empty body for method %s" (show m)
...
--       failA -< VRef addr
--     _ -> failA -< printf "Undefined Exception %s" clzz
--
...
--   case v of
--     IntVal n -> returnA -< n
--     _ -> failA -< "Expected an integer array index"
--
...
--   Nothing -> failA -< printf "Undefined label: %s" label
--
...
--   [] -> failA -< printf "No cases match value %s" (show v)
--
...
--   ThisRef -> do
--     (MethodSignature c _ _ _) <- getCurrentMethod -< ()
--     returnA -< (RefType c)
--   ParameterRef n -> do
--     (MethodSignature _ _ _ parameters) <- getCurrentMethod -< ()
--     returnA -< parameters !! n
...
--   FieldRef _ (FieldSignature _ t _) -> returnA -< t
--   SignatureRef (FieldSignature _ t _) -> returnA -< t
...
--   -- https://github.com/Sable/soot/blob/develop/src/main/java/soot/jimple/toolkits/typing/fast/AugEvalFunction.java
--
...
--                 write -< (addr, VObject c m')
--                 runStatements -< (stmts, i + 1)
--               Nothing -> failA -< printf "FieldSignature %s not defined on object %s: (%s)" (show fieldSignature) (show localName) (show m)
--           SignatureRef fieldSignature -> do
--             (addr, _) <- fetchFieldWithAddr -< fieldSignature
--             write -< (addr, v)
--             runStatements -< (stmts, i + 1)
--           _ -> failA -< printf "Can only assign a reference or a local variable"
--     If e label -> do
--       v <- eval -< e
--       case v of
