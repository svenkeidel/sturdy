...
spec = do
  ...
  describe "Simple Expressions" $ do
    ...
    it "8 / 0" $ do
      ...
      eval'' [] expr `shouldBe` SuccessOrFail (fromString "Expected numeric variables for 'div'") NonNull
    it "3.0 % 2.5" $ do
  ...
  describe "Simple Statements" $ do
    ...
    it "assign non-declared variable" $ do
      ...
      runStatements'' [] stmts `shouldBe` Fail (fromString "Variable \"s\" not bound")
  ...
  describe "Sound interpretation" $ jimpleSoundness
    (Con.evalImmediate' baseCompilationUnits) evalImmediate''
  ...
  where
    ...
    evalBool'' = evalBool' baseCompilationUnits
