...
spec = do
  describe "Literals" $ do
    it "LocalName lookup" $ do
      ...
      let nv = [("x", 1)]
      let st = [(1, int 2 2)]
      evalConcrete nv st expr `shouldSatisfy` ifSuccess (int 2 2)
  ...
  describe "Simple Expressions" $ do
    ...
    it "lengthof [1, 2, 3]" $ do
      ...
      evalConcrete nv st expr `shouldSatisfy` ifSuccess (int 3 3)
    it "8 + 2" $ do
      let ?bound = I.Interval (-100) 100
    ...
    it "[1, 2, 3][2]" $ do
      ...
      let st = [(1, RefVal 2),
                (2, ArrayVal (int 1 3) (int 3 3))]
      evalConcrete nv st expr `shouldSatisfy` ifSuccess (int 1 3)
    it "newmultiarray (float) [3][2]" $ do
      let ?bound = I.Interval (-100) 100
      let expr = NewMultiArrayExpr FloatType [IntConstant 3, IntConstant 2]
      evalConcrete env store expr `shouldSatisfy` ifSuccess (ArrayVal (ArrayVal (FloatVal 0.0) (int 2 2)) (int 3 3))
  ...
  describe "Simple Statements" $ do
    it "i0 = 2 + 3; return i0;" $ do
      ...
      runStatementsConcrete nv st stmts `shouldSatisfy` ifSuccess (Just (int 5 5))
    ...
    it "s = 2; xs = newarray (int)[s]; y = lengthof xs; return xs;" $ do
      ...
      let stmts = [Assign (LocalVar "s") (IntConstant 2),
                   ...
                   Return (Just (Local "y"))]
      runStatementsConcrete nv st stmts `shouldSatisfy` ifSuccess (Just (int 2 2))
    ...
    it "lookupswitch(4) { case 0: goto l1; case 4: goto l2; default: goto l3;}; l1: return 1; l2: return 2; l3: return 3;" $ do
      ...
      runStatementsConcrete env store stmts `shouldSatisfy` ifSuccess (Just (int 2 2))
    it "lookupswitch(2) { case 0: goto l1; case 4: goto l2; default: goto l3;}; l1: return 1; l2: return 2; l3: return 3;" $ do
      ...
      let stmts = [Lookupswitch (IntConstant 2) [(ConstantCase 0, "l1"),
                   ...
                   Label "l3",
                   Return (Just (IntConstant 3))]
      runStatementsConcrete env store stmts `shouldSatisfy` ifSuccess (Just (int 3 3))
    it "f0 := @parameter0: float; f1 = f0 * 2.5; return f1; (@parameter0 = 2.0)" $ do
      ...
      runStatementsConcrete nv st stmts `shouldSatisfy` ifSuccess (Just (FloatVal 5.0))

  describe "Complete program" $ do
    it "10! = 3628800" $ do
      let ?bound = I.Interval (-100) 4000000
      let files = baseCompilationUnits ++ [factorialExampleFile]
      runProgramConcrete files factorialExampleFile [IntConstant 10] `shouldSatisfy` ifSuccess (Just (int 3628800 3628800))
    it "s = new SingleMethodExample; s.x = 2; return s.x" $ do
      let ?bound = I.Interval (-100) 100
      let files = baseCompilationUnits ++ [singleMethodExampleFile]
      runProgramConcrete files singleMethodExampleFile [] `shouldSatisfy` ifSuccess (Just (int 2 2))
    ...
    it "5 -> [5,10,5,10]" $ do
      ...
      runProgramConcrete files arrayFieldExampleFile [IntConstant 5] `shouldSatisfy` ifSuccess (Just (ArrayVal (int 0 10) (int 4 4)))
    it "(new Person(10)).yearsToLive() = 90" $ do
      let ?bound = I.Interval (-100) 100
      let files = baseCompilationUnits ++ [personExampleFile]
      runProgramConcrete files personExampleFile [] `shouldSatisfy` ifSuccess (Just (int 90 90))
