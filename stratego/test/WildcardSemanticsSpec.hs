...
module WildcardSemanticsSpec(spec) where
...
--         if not (null vars) then elements (toList vars) else arbitrary
--       return $ counterexample
...
--     sound' s xs = sound M.empty (C.fromFoldable $ fmap (second termEnv) xs) (eval' s) (eval' s :: W.Interp W.Stack W.Term W.Term)
