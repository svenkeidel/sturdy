  ...
  describe "Scope" $ do
    ...
    it "should make non-declared variables available" $ do
      ...
      geval 2 (Scope ["y"] (Build "x")) tenv (numLit 42) `shouldBe`
        success (tenv, numLit 42)
      geval 2 (Scope ["y"] (Match "z")) tenv (numLit 42) `shouldBe`
        success (termEnv [("x", numLit 42), ("z", numLit 42)], numLit 42)
  ...
  describe "Call" $ do
    it "should apply a single function call" $ do
      let senv = LM.fromList [("swap", Closure swap' LM.empty)]
          t = grammar "S" [ ("S", Constr [("Tuple",["F", "G"])] )
                          , ("F", StringLit "foo" )
                          , ("G", StringLit "bar")
                          ]
                          []
