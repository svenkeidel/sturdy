...
module FlowInsensitiveIntervalAnalysisSpec where

import Prelude hiding ((<),Bool(..))
import Syntax
import IntervalAnalysis (Val(..))
import FlowInsensitiveIntervalAnalysis
...
spec = do
  it "x:=2; y:=4; x:=1; if(y>x) {z:=y} {z:=y*y}; x:=z" $ do
    ...
    run 3 [("x",0),("y",1),("z",2)] ["x" =: 2, "y" =: 4, "x" =: 1, ifExpr ("x" ~= "y") ["z" =: "y"] ["z" =: "y" * "y"], "x" =: "z"]
      `shouldBe` [(0,num 16 16), (1,num 4 4), (2,num 16 16)]
