...
type Store = Store Addr Val

-- Interp c x y ~= c (Store,(Env Text Addr,(ProgState,x))) (Error String (Store,(ProgState,y)))
newtype Interp c x y = Interp (State ProgState (Environment Text Addr (StoreArrow Addr Val (Except String c))) x y)

runInterp :: ArrowChoice c => Interp c x y -> c (Store, (Env, (ProgState,x))) (Error String (Store, (ProgState,y)))
...
deriving instance ArrowChoice c => ArrowStore Label (Interp c)
...
type instance Fix x y (Interp c) = Interp (Fix (Store,(Env,(ProgState,x))) (Error String (Store,(ProgState,y))) c)
deriving instance (ArrowFix (Store,(Env,(ProgState,x))) (Error String (Store,(ProgState,y))) c, ArrowChoice c) => ArrowFix x y (Interp c)
