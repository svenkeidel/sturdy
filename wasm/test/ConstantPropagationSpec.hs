...
runFunc modName funcName args = do
    ...
    Right (modInst, staticS, mems, tabs) <- instantiateAbstract m
    return $ invokeExported staticS tabs modInst mems (pack funcName) args
...
succResult (_,(_,(Terminating (_,(_,(_,(Exc.Success (_,result)))))))) = result
succResult res = error $ "unsuccessful result " ++ show res

terminatedSucc :: Result -> Bool
terminatedSucc (_, (errs, Terminating (_,(_,(_,(Exc.Success _)))))) = HashSet.null (toSet errs)
...
terminatedMaybeErr (_,(errs, Terminating (_,(_,(_,(Exc.Success _)))))) = not $ HashSet.null (toSet errs)
...
spec = do
    ...
    it "run test-size" $ do
        ...
        succResult result `shouldBe` [A.constantValue $ Wasm.VI32 1]

    it "run test-memgrow" $ do
        result <- runFunc "simple" "test-memgrow" []
        succResult result `shouldBe` [A.constantValue $ Wasm.VI32 1, A.constantValue $ Wasm.VI32 2]
